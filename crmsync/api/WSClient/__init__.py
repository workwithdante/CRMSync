...
class VTigerWSClient:
    def __init__(self, url):
        ...
        if url.endswith('/') is False:
            url += '/'
        if url.endswith(self._servicebase) is False:
            url += self._servicebase
    ...
    def __doChallenge(self, username):
        parameters = {'operation': 'getchallenge', 'username': username}
    ...
    def doLogin(self, username, accesskey):
        if not self.__doChallenge(username):
            return False
        parameters = {
            'operation': 'login',
            'username': username,
    ...
    def doListTypes(self):
        if not self.__checkLogin():
            return False
        parameters = {'operation': 'listtypes', 'sessionName': self._sessionid}
        response = self.__doGet(self._serviceurl, parameters)
    ...
    def doQuery(self, query):
        ...
        parameters = {'operation': 'query', 'sessionName': self._sessionid, 'query': sanitized_query}
        response = self.__doGet(self._serviceurl, parameters)
        if self.hasError(response):
            return False
        result = response['result']
    ...
    def doDescribe(self, module):
        if not self.__checkLogin():
            return False
        parameters = {'operation': 'describe', 'sessionName': self._sessionid, 'elementType': module}
        response = self.__doGet(self._serviceurl, parameters)
        if self.hasError(response):
            return False
        result = response['result']
    ...
    def doRetrieve(self, record):
        if not self.__checkLogin():
            return False
        parameters = {'operation': 'retrieve', 'sessionName': self._sessionid, 'id': record}
        response = self.__doGet(self._serviceurl, parameters)
        if self.hasError(response):
            return False
        result = response['result']
    ...
    def doInvoke(self, method, params=False, type='POST'):
        ...
        parameters = {'operation': method, 'sessionName': self._sessionid}
